...
- **Margins Of Error (MOE)** are our baseline measurement of error. The
  ACS reports MOEs at a 90% confidence level, which enables us to say
  something like: “20% (±5%) of households in the neighborhood have no
...
- **Coefficients of Variation (CV)** relate error to the size of the
  ...
  unit-agnostic measurement of variable quality: a CV of 40 for a given
  variable means the same thing as a CV of 40 for another unrelated
  variable, whereas MOEs and SEs are not directly comparable across
  variables.
...
to conduct our actual tests. Here, we’ll compare each tract-level value
...
[`library(ggdist)`](https://mjskay.github.io/ggdist/) provides an array
of methods for different styles and methods of visualizing error, while
visualizing statistical significance relative to a benchmark (see the
map above) is another common approach.
...
can inform users’ understandings of the number of raw estimates used to
