...
Another challenge is that much decisionmaking occurs at geographies that
are not supported directly by the ACS. Think, for example, of school
districts, political wards, a city’s neighborhoods, or the
unincorporated area governed by a county. To translate (or
“interpolate”) data to these geographies can require significant work,
...
both ACS estimates and MOEs to user-defined geographies. It requires a
crosswalk that specifies the share of source geography values that
should be allocated to each target geography, though for the special
case where source geographies are perfectly nested within target
geographies (e.g., tracts within counties), the function assigns weights
of one to every observation (the case when `weight = NULL`). Note that
[`interpolate_acs()`](https://ui-research.github.io/urbnindicators/reference/interpolate_acs.md)
always returns a non-spatial dataframe, so geometry must be managed
separately.
...
We’ll demonstrate
[`interpolate_acs()`](https://ui-research.github.io/urbnindicators/reference/interpolate_acs.md)
first using tract-level data for Washington, DC, comparing the share of
population receiving SNAP benefits across “quadrants”. We pull tract
data with `spatial = TRUE` because we need the geometry both for
...
For illustration, we’ll create four quadrants of DC by assigning tracts
...
Next, dissolve the tract geometries into quadrant boundaries. We do this
*before* calling
[`interpolate_acs()`](https://ui-research.github.io/urbnindicators/reference/interpolate_acs.md)
...
Now aggregate the data with
[`interpolate_acs()`](https://ui-research.github.io/urbnindicators/reference/interpolate_acs.md).
With `weight = NULL` (the default), it sums count variables,
...
![](custom-geographies_files/figure-html/unnamed-chunk-7-1.png)

Indeed, the median Coefficient of Variation (CV; which reflects the size
of the MOE relative to the size of the estimate) at the tract level
exceeds 30—a common upper bound for “reliable” estimates—while
...
Tract-level median CV: 37. Quadrant-level median CV: 7.

## Detecting Statistically Significant Differences

At the tract level, many tracts are not statistically significantly
different as compared to the mean across all DC tracts. By aggregating
our tract observations, we can detect statistically significant
differences at greater geographic scales. This enables analysis for more
policy-relevant areas and helps mitigate shortcomings associated with
...
![](custom-geographies_files/figure-html/unnamed-chunk-9-1.png)
...
quadrants we define. But target geographies don’t always align neatly
with source geography boundaries. When source geographies partially
overlap targets, you can use a crosswalk with weights to allocate data
from source geographies to target geographies proportionally. For
example. a common use-case is aligning 2010-vintage tracts and
...
other sources. We’ll pull tract-level SNAP data for both 2019 (which
...
tract_xwalk = tract_crosswalk$crosswalks$step_1 %>%
  filter(weighting_factor == "population") %>%
  select(
    GEOID = source_geoid,
