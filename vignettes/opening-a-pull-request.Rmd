...
declares the raw ACS variables and uses the `define_*` DSL to specify
how derived variables are computed. The codebook, coefficients of
variation, and variable listings are all generated automatically from
this registration.
...
4.  **Pretty names**: Verify that `make_pretty_names()` produces
    reasonable labels.
...
across three files (`R/list_acs_variables.R`,
`R/compile_acs_data.R`, and manual codebook verification).
...
the `utils::globalVariables()` call at the bottom of
`R/table_registry.R` to avoid R CMD check notes.
...
codebook attribute of the dataframe returned by `compile_acs_data()`.
...
Integrate the proposed changes into the codebase (on a branch), load
the current version of `urbnindicators` (via `devtools::load_all()`),
and call `compile_acs_data()`. Verify that the new table works both in
...
2.  Manually compute a benchmark value. This works well for all those
    derived variables that aren't reported in any Census Bureau
    product. Identify the relevant numerator and denominator variables
...
3.  Plot a histogram of the computed variable(s) (if multiple
