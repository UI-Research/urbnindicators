...
2.  We show how `urbnindicators` can produce similar outputs, but with
    much less code and effort.

3.  We highlight how `urbnindicators` provides metadata and improves the
...
`urbnindicators` is built on top of `tidycensus`, and we highly
encourage those who are unfamiliar to explore `tidycensus` before using
`urbnindicators`.
...
Here's a typical `tidycensus` workflow:
...
combine the margins of error that are returned by `get_acs()`, opting
simply to drop this critical information from their analysis as they go
about calculating "% Disabled".

## Using urbnindicators

`urbnindicators` abstracts the workflow above behind the scenes. Instead
...
`urbnindicators` includes a codebook as an attribute of the dataframe
returned from `compile_acs_data()`.
...
are calculated by `urbnindicators` are percent variables, where we
...
from home. This allows us say something along the lines of: "Of
...
2.  Reliability:
    ...
    3.  Behind-the-scenes quality checks help to ensure that
        calculations are correct (though users still should verify
        results themselves!).
